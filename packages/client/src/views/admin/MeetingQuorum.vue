...
async function loadVoters(): Promise<void> {
	...
	try {
		const response = await getQuorumVoters(meetingId);
		if (response.data !== undefined) {
...
async function callQuorum(): Promise<void> {
	...
	try {
		const response = await updateQuorum(meetingId, new Date().toISOString());
		if (response.data !== undefined) {
			report.value = response.data;
		}
		// Stop auto-refresh since quorum is now called
		stopAutoRefresh();
		// Reload voters to get the frozen list
		await loadVoters();
	} catch (err) {
		error.value = err instanceof Error ? err.message : "Failed to call quorum";
	} finally {
		calling.value = false;
	}
}

async function uncallQuorum(): Promise<void> {
	if (calling.value) return;
	...
	try {
		const response = await updateQuorum(meetingId, null);
		if (response.data !== undefined) {
			report.value = response.data;
		}
		// Start auto-refresh since we're back to live counting
		startAutoRefresh();
	} catch (err) {
		error.value =
			err instanceof Error ? err.message : "Failed to uncall quorum";
	} finally {
		calling.value = false;
	}
}
		...
		<template v-else-if="report !== null">
			...
			<div class="quorum-actions">
				...
				<button
					...
					@click="uncallQuorum"
				>
					{{ calling ? "Uncalling..." : "Uncall Quorum" }}
				</button>
			</div>
